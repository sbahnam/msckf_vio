...
#include "act_fast.hpp"
#include <math.h>
#include "image.hpp"
#include "../../opticalflow/opticflow_calculator.hpp"
#include <iostream>
#include <chrono>
...
*/
void act_fast(const cv::Mat* img, uint8_t fast_threshold, uint16_t *num_corners, struct point_t **ret_corners,
              uint16_t n_agents, uint16_t n_time_steps, float long_step, float short_step, int min_gradient, int gradient_method)
{
  ...
  float step_size_x = (img->cols - 2 * init_border) / (GRID_ROWS - 1);
  float step_size_y = (img->rows - 2 * init_border) / (GRID_ROWS - 1);
  ...
  for (int c = 0; c < GRID_ROWS; c++) {
    for (int r = 0; r < GRID_ROWS; r++) {
      ...
      px = ((float)(rand() % 10000) + 1) / 10000.0f - 0.5; //also initialize with prefered negative direction
      py = ((float)(rand() % 10000) + 1) / 10000.0f - 0.5; //also initialize with prefered negative direction
  ...
  for (int t = 0; t < n_time_steps; t++) {
    ...
    for (a = 0; a < n_agents; a++) {
      ...
      if (agents[a].active) {
        ...
        if (agents[a].x > img->cols - border) {
          agents[a].x = border;
        } else if (agents[a].x < border) {
          agents[a].x = img->cols - border;
        }
        if (agents[a].y > img->rows - border) {
          agents[a].y = border;
        } else if (agents[a].y < border) {
          agents[a].y = img->rows - border;
        }
      }
    }
  }
...
// int main(){
//   std::string image_path = "/home/stavrow/KITTI_dataset/dataset/sequences/03/image_0/000000.png";
//   cv::Mat imgOriginal = cv::imread(image_path);
//   cv::Mat img;
...
//   cv::cvtColor(imgOriginal, img, CV_BGR2GRAY);        // convert to graysca
//   // cv::Mat img = getInput<cv::Mat> ( m_leftImgId_str, cv::Mat() )
//   uint8_t fast_threshold = 12;
//   uint16_t num_corners = 0;
//   struct point_t* ret_corners;
//   // struct point_t* ret_corners_p = &ret_corners;
//   ret_corners =  (struct point_t*) calloc(MAX_AGENTS, sizeof(struct point_t));
  
//   uint16_t n_agents = 50;
...
//               n_agents, n_time_steps, long_step, short_step, min_gradient, gradient_method);
  
//   auto finish = std::chrono::high_resolution_clock::now();
//   std::cout<<"Time duration: "<< std::chrono::duration_cast<std::chrono::nanoseconds>(finish-start).count() <<std::endl;
//   std::cout<<"Number of corners: "<<num_corners<<std::endl;
//   return 0;
// }