 ...
 */
void image_gradient_pixel(const cv::Mat* img, struct point_t *loc, int method, int *dx, int *dy)
{
  ...
  if (loc->x >= 1 && (loc->x + 1) < img->cols && loc->y >= 1 && (loc->y + 1) < img->rows) {
    if (method == 0) {
      ...
      // dx:
      // index = loc->y * img->cols * pixel_width + (loc->x - 1) * pixel_width;
      gradient_x -= (int) img->at<uchar>(loc->y, loc->x -1);
      // index = loc->y * img->cols * pixel_width + (loc->x + 1) * pixel_width;
      gradient_x += (int) img->at<uchar>(loc->y, loc->x +1);
      // dy:
      // index = (loc->y - 1) * img->cols * pixel_width + loc->x * pixel_width;
      gradient_y -= (int) img->at<uchar>(loc->y -1, loc->x);
      // index = (loc->y + 1) * img->cols * pixel_width + loc->x * pixel_width;
      gradient_y += (int) img->at<uchar>(loc->y +1, loc->x);
    } else {
      ...
      for (int x = -1; x <= 1; x++) {
        for (int y = -1; y <= 1; y++) {
          // index = (loc->y + y) * img->cols * pixel_width + (loc->x + x) * pixel_width;
          if (x != 0) {
            filt_ind_x = (x + 1) % 3 + (y + 1) * 3;
            gradient_x += Sobel[filt_ind_x] * img->at<uchar>(loc->y + y, loc->x + x);
          }
          if (y != 0) {
            gradient_y += Sobel[filt_ind_y] * img->at<uchar>(loc->y + y, loc->x + x);
          }
          filt_ind_y++;
        }
      }
